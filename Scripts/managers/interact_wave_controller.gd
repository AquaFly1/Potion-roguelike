...
func end_wave():
	...
	tween_adjustments(1,0.0)

func tween_adjustments(amount:float, warp_amount: float = amount):
	for i in ["adjustment_brightness","adjustment_saturation","adjustment_contrast"]:
		tween.tween_property(Player.node.camera.environment,i,
			(battle_curves.get(i)-Player.node.camera.environment.get(i))*amount + Player.node.camera.environment.get(i)
			,1)
